...
const Select = styled.select`
  ...
  /* webkit le quita la apariencia por default y permite añadir lo que quieras a posteriori */
...
const Formulario = ({ setResumen }) => {
  ...
  //Leer los datos del formulario y colocarlose en el state

  const obtenerInformacion = (event) => {
    const { name, value } = event.target;
    setDatos({ ...datos, [name]: value });
  };
  //cuando el usuario presiona submnit
  const cotizarSeguro = (event) => {
    ...
    //por cada año bajar un 3% del valor
    ...
    <form onSubmit={cotizarSeguro}>
      ...
      <Campo>
        ...
        <InputRadio
          ...
          name="plan"
          value="basico"
          //   se añade el checked para que en este caso sepa cuando marcamos uno u otro
          checked={plan === "basico"}
